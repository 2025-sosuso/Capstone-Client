...
import {VideoSummaryItem} from "@/types";
...
export default function VideoPreviewList({type = "trending", data}: Props) {
    ...
    return (
        <div className="flex flex-col gap-5">
            {data.map((item, i) => (
                <VideoPreview key={item.video?.id ?? i} rank={i + 1} data={item}/>
            ))}
        </div>
    );
}
